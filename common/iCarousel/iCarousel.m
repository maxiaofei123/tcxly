...
- (CGFloat)valueForOption:(iCarouselOption)option withDefault:(CGFloat)value
{
    ...
    switch (option)
    {
        case iCarouselOptionWrap:
        {
            if ([_delegate respondsToSelector:@selector(carouselShouldWrap:)])
            {
                return [(id<iCarouselDelegate>)_delegate carousel:self valueForOption:iCarouselOptionWrap withDefault:value];
...
//                return [(id<iCarouselDeprecated>)_delegate carouselOffsetMultiplier:self];
                return [(id<iCarouselDelegate>)_delegate carousel:self valueForOption:iCarouselOptionOffsetMultiplier withDefault:value];
            }
            break;
        }
        default:
